...
public class intakeOut extends Command {
    ...
    public void initialize(){
        if (isHolding && !m_Conditions.hasCycled){
            m_IntakeArmSubsystem.setIntakeArmPosition(-9.8);
            m_IntakeRollerSubsystem.intakeRollers(1,true);
            new startCycle(m_IntakeArmSubsystem, m_IntakeRollerSubsystem, m_ShooterArmSubsystem, m_ShooterRollersSubsystem).schedule();
        }else if (!isHolding && !m_Conditions.hasCycled){
            m_IntakeArmSubsystem.setIntakeArmPosition(0);
